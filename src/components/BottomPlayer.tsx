...
import { Play, Pause, SkipBack, SkipForward, Volume2, Mic2, ChevronDown, Music } from 'lucide-react'
import { Button } from '@/components/ui/button'
import DraggableSlider from '@/components/ui/draggable-slider'

export default function BottomPlayer() {
  const { currentSong, isPlaying, play, pause, volume, setVolume, isLyricsOpen, toggleLyrics } = usePlayerStore()
  ...
  return (
    <>
      {/* THE LYRICS OVERLAY (Slides up from the bottom) */}
      <div 
      ...
      >
        ...
        <div className="w-full md:w-1/3 flex flex-col gap-6 md:sticky md:top-10 mt-10 md:mt-0">
          ...
          <div className="flex flex-col">
            ...
            {currentSong.author && <p className="text-sm text-slate-500 mt-2">Written by {currentSong.author}</p>}
          </div>
        </div>

        {/* Right Column: The Lyrics Text */}
      ...
      <div className="fixed bottom-0 left-0 right-0 h-24 bg-slate-950 border-t border-slate-800 text-white flex items-center px-4 md:px-8 z-50">
        ...
        {/* Center Area: Controls & Progress Bar */}
